...
int	get_y(t_render *game, int y, t_pixel *pxl)
{
	...
	if (pxl->wall_start >= 0)
	{
		...
		{
			mlx_put_pixel(game->text.img_backgrnd, pxl->x, y, game->text.ceiling_color);
			y++;
		}
	}
	else
...
void	pxl_to_buffer(t_render *game, t_pixel pxl)
{
    ...
    {
		mlx_put_pixel(game->text.img_backgrnd, pxl.x, y, pixel_from_texture(game, pxl, (size_t)pxl.wall_y));
		pxl.wall_y += pxl.wall_step;
		y++;
    }
	while (y < HEIGHT - 1)
	{
		mlx_put_pixel(game->text.img_backgrnd, pxl.x, y, game->text.floor_color);
		y++;
	}
}
